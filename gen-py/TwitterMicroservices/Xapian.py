...
class Client(Iface):
    ...
    def search(self, key, req_id):
        ...
        Parameters:
         ...
         - req_id
        """
        self.send_search(key, req_id)
        return self.recv_search()

    def send_search(self, key, req_id):
...
class Processor(Iface, TProcessor):
    ...
    def process_search(self, seqid, iprot, oprot):
        ...
        result = search_result()
        try:
            result.success = self._handler.search(args.key, args.req_id)
            msg_type = TMessageType.REPLY
        except (TTransport.TTransportException, KeyboardInterrupt, SystemExit):
            raise
        except Exception as ex:
...
class search_args(object):
    ...
    def __init__(self, key=None, req_id=None,):
        self.key = key
        self.req_id = req_id
