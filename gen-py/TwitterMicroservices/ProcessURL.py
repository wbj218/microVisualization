...
class Iface(object):
    ...
    def process_url(self, req_id, text_data, type):
        """
        Parameters:
         - req_id
         - text_data
...
class Client(Iface):
    ...
    def process_url(self, req_id, text_data, type):
        ...
        Parameters:
         ...
         - type
        """
        self.send_process_url(req_id, text_data, type)

    def send_process_url(self, req_id, text_data, type):
        self._oprot.writeMessageBegin('process_url', TMessageType.ONEWAY, self._seqid)
        args = process_url_args()
        args.req_id = req_id
...
class Processor(Iface, TProcessor):
    def __init__(self, handler):
        ...
        self._processMap["process_url"] = Processor.process_process_url
    ...
    def process_process_url(self, seqid, iprot, oprot):
        args = process_url_args()
        args.read(iprot)
        iprot.readMessageEnd()
        try:
            self._handler.process_url(args.req_id, args.text_data, args.type)
        except (TTransport.TTransportException, KeyboardInterrupt, SystemExit):
            raise
        except:
            pass
...
class process_url_args(object):
    ...
    def write(self, oprot):
        if oprot._fast_encode is not None and self.thrift_spec is not None:
            oprot.trans.write(oprot._fast_encode(self, (self.__class__, self.thrift_spec)))
            return
        oprot.writeStructBegin('process_url_args')
