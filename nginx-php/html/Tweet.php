...
try {   
    ...
    $uniqueid_client->get_unique_id($req_id, "TWEET");    
    $uniqueid_transport->close();

    if ($_POST["text"]) {

        $urlshortener_transport->open();        
        $urlshortener_client->process_url($req_id, $_POST['text'], "TWEET");    
        $urlshortener_transport->close();
        
        $tag_transport->open();
        $tag_client->process_tag($req_id, $_POST['text'], "TWEET"); 
        $tag_transport->close();
        
        $text_transport->open();        
        $text_client->process_text($req_id, $_POST['text'], "TWEET"); 
    ...
    if ($_POST["image"]) {
        ...
        $image_client->process_image($req_id, $_POST['image'], "TWEET");    
        $image_transport->close();
    }
    else{
        $image_transport->open();       
        $image_client->process_image($req_id, "", "TWEET");    
        $image_transport->close();
    }
    ...
    if ($_POST["video"]) {
        ...
        // echo $_POST["video"];        
        $video_client->process_video($req_id, $_POST['video'], "TWEET");    
        $video_transport->close();
    }
    else{
        $video_transport->open();       
        $video_client->process_video($req_id, "", "TWEET");    
        $video_transport->close();
    }
