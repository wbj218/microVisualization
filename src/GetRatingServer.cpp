...
void GetRatingHandler::get_rating(const std::string& req_id, const std::string& movie_id) {
    if (IF_TRACE)
    ...
    try {
        ...
        compose_page_client[compose_page_index]->upload_rating(req_id, movie_id, rating);
