...
public:
    GetRatingHandler(const int n_movie_info_store);
...
GetRatingHandler::GetRatingHandler(const int n_movie_info_store) {
...
void GetRatingHandler::get_rating(std::string& _return, const std::string& req_id, const std::string& movie_id) {
    if (IF_TRACE)
    ...
    try {
        ...
        store_client[store_index]->get_info(_return, req_id, movie_id, "rating");
...
int main(int argc, char *argv[]) {
    ...
    TSimpleServer server(
            boost::make_shared<GetRatingProcessor>(boost::make_shared<GetRatingHandler>(n_store)),
