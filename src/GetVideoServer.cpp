...
public:
    GetVideoHandler(const int n_movie_info_store);
...
GetVideoHandler::GetVideoHandler(const int n_movie_info_store) {
...
void GetVideoHandler::get_video(std::string& _return, const std::string& req_id, const std::string& movie_id) {
    if (IF_TRACE)
    ...
    try {
        ...
        store_client[store_index]->get_info(_return, req_id, movie_id, "video");
...
int main(int argc, char *argv[]) {
    ...
    TSimpleServer server(
            boost::make_shared<GetVideoProcessor>(boost::make_shared<GetVideoHandler>(n_store)),
