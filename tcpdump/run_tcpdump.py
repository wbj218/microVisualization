...
#             + " > ./logs/mmc_movie_db_" + str(i) + ".log &")   
    sp.call("sudo tcpdump -i p2p1 -A -tt -nn port " + str(MMC_MOVIE_DB_PORT + i)
            + " > ./logs/mmc_movie_db_" + str(i) + ".log &", shell = True)
    sp.call("sudo tcpdump -i p2p1 -A -tt -nn port " + str(MONGO_MOVIE_DB_PORT + i)
            + " > ./logs/mongo_movie_db_" + str(i) + ".log &", shell = True)

for i in range(NUM_USERS):
    sp.call("sudo tcpdump -i p2p1 -A -tt -nn port " + str(MMC_USER_DB_PORT + i)
            + " > ./logs/mmc_user_db_" + str(i) + ".log &", shell = True)
    sp.call("sudo tcpdump -i p2p1 -A -tt -nn port " + str(MONGO_USER_DB_PORT + i)
            + " > ./logs/mongo_user_db_" + str(i) + ".log &", shell = True)

sp.call("sudo tcpdump -i p2p1 -A -tt -nn port " + str(MMC_REVIEW_STORAGE_PORT)
        + " > ./logs/mmc_review_storage.log &", shell = True)
sp.call("sudo tcpdump -i p2p1 -A -tt -nn port " + str(MONGO_REVIEW_STORAGE_PORT)
